...
ifeq ($(OSD),retro)
ifeq ($(TARGETOS),win32)
EMULATOR = arcanetmame_libretro.dll
else
EMULATOR = arcanetmame_libretro.so
endif
endif
...
ifeq ($(OSD),retro)
ifeq ($(TARGETOS),win32)
LDFLAGSEMULATOR += -shared -static-libgcc -Wl,--version-script=$(SRC)/osd/retro/link.T
else
LDFLAGSEMULATOR += -shared -Wl,--version-script=$(SRC)/osd/retro/link.T
LIBS += -lm -lpthread
endif
endif
...
$(EMULATOR): maketree $(VERSIONOBJ) $(DRVLIBS) $(LIBOSD) $(LIBEMU) $(LIBCPU) $(LIBDASM) $(LIBSOUND) $(LIBUTIL) $(EXPAT) $(ZLIB) $(LIBOCORE) $(RESFILE)
# always recompile the version string
	$(CC) $(CDEFS) $(CFLAGS) -c $(SRC)/version.c -o $(VERSIONOBJ)
	@echo Linking $@...
	$(LD) $(LDFLAGS) $(LDFLAGSEMULATOR) -Wl,--start-group $(filter-out maketree,$^) -Wl,--end-group $(LIBS) -o $@
