...
int retro_make_directory(const char *path)
{
	char *copy;
	UINT32 result;
	...
	if (copy == NULL)
		return FALSE;
	strcpy(copy, path);
	result = create_path_recursive(copy);
	free(copy);
...
file_error osd_open(const char *path, UINT32 openflags, osd_file **file, UINT64 *filesize)
{
	...
	{
		char message[512];
		sprintf(message, "osd_open: %s flags=%08x", path, openflags);
		printf("%s\n", message);
	}
	#endif
	...
	if (fileptr == NULL){
		...
		if ((openflags & OPEN_FLAG_CREATE) && (openflags & OPEN_FLAG_CREATE_PATHS))
		{
			...
			if (pathsep != NULL)
			{
				...
				{
					char message[512];
					sprintf(message, "osd_open: create path %s result=%d", tmpstr, error);
					printf("%s\n", message);
				}
				#endif
...
static UINT32 create_path_recursive(char *path)
{
	...
	if (sep != NULL && sep > path && sep[-1] != PATHSEPCH
#ifdef WIN32
		&& !(sep == path + 2 && path[1] == ':')
#endif
		)
