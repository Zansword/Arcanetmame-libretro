...
static void init_libretro_pads(running_machine *machine)
{
   input_device *devices[8];
   UINT8 *states[8] = { P1_state, P2_state, P3_state, P4_state, P5_state, P6_state, P7_state, P8_state };
   int player;

   for (player = 0; player < 8; player++)
   {
      char name[16];
      sprintf(name, "Retropad%d", player + 1);
      devices[player] = input_device_add(machine, DEVICE_CLASS_JOYSTICK, name, NULL);
      if (devices[player] == NULL)
         fatalerror("RetroPad error creating device %d\n", player + 1);

      input_device_item_add(devices[player], "X", &retro_pad_axes[player][0], ITEM_ID_XAXIS, generic_axis_get_state);
      input_device_item_add(devices[player], "Y", &retro_pad_axes[player][1], ITEM_ID_YAXIS, generic_axis_get_state);
      input_device_item_add(devices[player], "RX", &retro_pad_axes[player][2], ITEM_ID_RXAXIS, generic_axis_get_state);
      input_device_item_add(devices[player], "RY", &retro_pad_axes[player][3], ITEM_ID_RYAXIS, generic_axis_get_state);
      input_device_item_add(devices[player], "Start", &states[player][KEY_START], ITEM_ID_START, pad1_get_state);
      input_device_item_add(devices[player], "Select", &states[player][KEY_COIN], ITEM_ID_SELECT, pad1_get_state);
      input_device_item_add(devices[player], "Button1", &states[player][KEY_BUTTON_1], ITEM_ID_BUTTON1, pad1_get_state);
      input_device_item_add(devices[player], "Button2", &states[player][KEY_BUTTON_2], ITEM_ID_BUTTON2, pad1_get_state);
      input_device_item_add(devices[player], "Button3", &states[player][KEY_BUTTON_3], ITEM_ID_BUTTON3, pad1_get_state);
      input_device_item_add(devices[player], "Button4", &states[player][KEY_BUTTON_4], ITEM_ID_BUTTON4, pad1_get_state);
      input_device_item_add(devices[player], "Button5", &states[player][KEY_BUTTON_5], ITEM_ID_BUTTON5, pad1_get_state);
      input_device_item_add(devices[player], "Button6", &states[player][KEY_BUTTON_6], ITEM_ID_BUTTON6, pad1_get_state);
   }
}
...
static void initInput(running_machine *machine)
{
   ...
   fprintf(stderr, "MANUFACTURER: %s\n", machine->gamedrv->manufacturer);
       ...
       P1_device=input_device_add(machine,DEVICE_CLASS_KEYBOARD,"RPad1",NULL);	
	if (P1_device == NULL){
		fatalerror("P1 Error creating keyboard device\n");
printf("P1 Error creating keyboard device\n");
        }
	...
	input_device_item_add_p2(P1_device, "F11", &P1_state[KEY_F11],ITEM_ID_ENTER/* ITEM_ID_F11*/, pad1_get_state);


	input_device_item_add_p2(P1_device, "Tab", &P1_state[KEY_TAB], ITEM_ID_TAB, pad1_get_state);
	input_device_item_add_p2(P1_device, "F3", &P1_state[KEY_F3], ITEM_ID_F3, pad1_get_state);
	input_device_item_add_p2(P1_device, "F2", &P1_state[KEY_F2], ITEM_ID_F2, pad1_get_state);
	input_device_item_add_p2(P1_device, "P1 Start", &P1_state[KEY_START], ITEM_ID_1, pad1_get_state);
	input_device_item_add_p2(P1_device, "COIN1", &P1_state[KEY_COIN], ITEM_ID_5, pad1_get_state);
	input_device_item_add_p2(P1_device, "P1 JoyU", &P1_state[KEY_JOYSTICK_U], ITEM_ID_UP, pad1_get_state);
	input_device_item_add_p2(P1_device, "P1 JoyD", &P1_state[KEY_JOYSTICK_D], ITEM_ID_DOWN, pad1_get_state);
	input_device_item_add_p2(P1_device, "P1 JoyL", &P1_state[KEY_JOYSTICK_L], ITEM_ID_LEFT, pad1_get_state);
	input_device_item_add_p2(P1_device, "P1 JoyR", &P1_state[KEY_JOYSTICK_R], ITEM_ID_RIGHT, pad1_get_state);
   ...
   else
   {
	...
	P2_device= input_device_add(machine,DEVICE_CLASS_KEYBOARD,"RPad2",NULL);
 	if (P2_device == NULL)
...
//printf("ppp %d\n",mame_get_phase(machine));
   ...
   {
      input_device *extra_device;
      UINT8 *extra_state[6] = { P3_state, P4_state, P5_state, P6_state, P7_state, P8_state };
      int extra_player;

      for (extra_player = 0; extra_player < 6; extra_player++)
      {
         char device_name[16];
         sprintf(device_name, "RPad%d", extra_player + 3);
         extra_device = input_device_add(machine, DEVICE_CLASS_KEYBOARD, device_name, NULL);
         if (extra_device == NULL)
            fatalerror("Extra RetroPad error creating device\n");

         input_device_item_add(extra_device, "Start", &extra_state[extra_player][KEY_START], ITEM_ID_1, pad1_get_state);
         input_device_item_add(extra_device, "Coin", &extra_state[extra_player][KEY_COIN], ITEM_ID_5, pad1_get_state);
         input_device_item_add(extra_device, "B1", &extra_state[extra_player][KEY_BUTTON_1], ITEM_ID_LCONTROL, pad1_get_state);
         input_device_item_add(extra_device, "B2", &extra_state[extra_player][KEY_BUTTON_2], ITEM_ID_LALT, pad1_get_state);
         input_device_item_add(extra_device, "B3", &extra_state[extra_player][KEY_BUTTON_3], ITEM_ID_SPACE, pad1_get_state);
         input_device_item_add(extra_device, "B4", &extra_state[extra_player][KEY_BUTTON_4], ITEM_ID_RCONTROL, pad1_get_state);
         input_device_item_add(extra_device, "B5", &extra_state[extra_player][KEY_BUTTON_5], ITEM_ID_RALT, pad1_get_state);
         input_device_item_add(extra_device, "B6", &extra_state[extra_player][KEY_BUTTON_6], ITEM_ID_Q, pad1_get_state);
         input_device_item_add(extra_device, "JoyU", &extra_state[extra_player][KEY_JOYSTICK_U], ITEM_ID_UP, pad1_get_state);
         input_device_item_add(extra_device, "JoyD", &extra_state[extra_player][KEY_JOYSTICK_D], ITEM_ID_DOWN, pad1_get_state);
         input_device_item_add(extra_device, "JoyL", &extra_state[extra_player][KEY_JOYSTICK_L], ITEM_ID_LEFT, pad1_get_state);
         input_device_item_add(extra_device, "JoyR", &extra_state[extra_player][KEY_JOYSTICK_R], ITEM_ID_RIGHT, pad1_get_state);
      }
   }
...
void retro_poll_mame_input(void)
{
	input_poll_cb();
   {
      ...
      for (player = 0; player < 8; player++)
      {
         INT32 axis_x = 2 * input_state_cb(player, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT, RETRO_DEVICE_ID_ANALOG_X);
         INT32 axis_y = 2 * input_state_cb(player, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT, RETRO_DEVICE_ID_ANALOG_Y);

         if (axis_x == 0)
            axis_x = states[player][KEY_JOYSTICK_L] ? -32767 : (states[player][KEY_JOYSTICK_R] ? 32767 : 0);
         if (axis_y == 0)
            axis_y = states[player][KEY_JOYSTICK_U] ? -32767 : (states[player][KEY_JOYSTICK_D] ? 32767 : 0);

         retro_pad_axes[player][0] = axis_x;
         retro_pad_axes[player][1] = axis_y;
         retro_pad_axes[player][2] = 2 * input_state_cb(player, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT, RETRO_DEVICE_ID_ANALOG_X);
         retro_pad_axes[player][3] = 2 * input_state_cb(player, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT, RETRO_DEVICE_ID_ANALOG_Y);
      }
   }
...
// process_keyboard_state
...

   {
      ...
      for (extra_player = 0; extra_player < 6; extra_player++)
      {
         UINT8 *state = extra_state[extra_player];
         unsigned port = extra_player + 2;

         state[KEY_START] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_START);
         state[KEY_COIN] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_SELECT);
         state[KEY_BUTTON_1] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_A);
         state[KEY_BUTTON_2] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_B);
         state[KEY_BUTTON_3] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X);
         state[KEY_BUTTON_4] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_Y);
         state[KEY_BUTTON_5] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L);
         state[KEY_BUTTON_6] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R);
         state[KEY_JOYSTICK_U] = input_state_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_UP);
