...
extern void retro_debug_log(const char *message);
#ifdef __LIBRETRO__
extern int RLOOP;
#endif
...
static int retro_machine_exit_in_progress;

static void retro_machine_exit(running_machine *machine)
{
	mame_private *mame;
	callback_item *cb;

	if (machine == NULL || machine->mame_data == NULL || retro_machine_exit_in_progress)
		return;

	retro_machine_exit_in_progress = TRUE;
	mame = machine->mame_data;
	mame->current_phase = MAME_PHASE_EXIT;
	mame->fatal_error_jmpbuf_valid = FALSE;
	retro_debug_log("retro_machine_exit: before nvram save");
	nvram_save(machine);
	retro_debug_log("retro_machine_exit: after nvram save");
	retro_debug_log("retro_machine_exit: before config save");
	config_save_settings(machine);
	retro_debug_log("retro_machine_exit: after config save");
	retro_debug_log("retro_machine_exit: before sound mute");
	sound_mute(machine, TRUE);
	retro_debug_log("retro_machine_exit: after sound mute");
	end_resource_tracking();
	retro_debug_log("retro_machine_exit: after resource tracking");

	for (cb = mame->exit_callback_list; cb != NULL; cb = cb->next)
	...
	retro_machine_exit_in_progress = FALSE;
}
...
/* libretro drives one emulation timeslice from each retro_run call. */
void retro_main_loop(void)
{
	running_machine *machine = global_machine;
	mame_private *mame;

	if (machine == NULL || machine->mame_data == NULL)
		return;

	mame = machine->mame_data;
	RLOOP = 1;
	while (RLOOP == 1 && !mame->exit_pending && !mame->hard_reset_pending)
	{
		if (!mame->paused)
	...
	if ((mame->exit_pending || mame->hard_reset_pending) &&
		mame->saveload_pending_file == NULL && global_machine != NULL)
	{
		retro_debug_log("retro_main_loop: exiting machine");
		retro_machine_exit(machine);
	}
}

/* Release a libretro-owned machine without entering the standalone CLI loop. */
...
int mame_execute(core_options *options)
{
	...
	while (error == MAMERR_NONE && !exit_pending)
	{
		...
		retro_debug_log("mame_execute: driver lookup complete");
		...
		retro_debug_log("mame_execute: validity complete");
		firstgame = FALSE;

		/* parse any INI files as the first thing */
		retro_debug_log("mame_execute: before parse ini");
		options_revert(mame_options(), OPTION_PRIORITY_INI);
		mame_parse_ini_files(mame_options(), driver);
		retro_debug_log("mame_execute: after parse ini");

		/* create the machine structure and driver */
		machine = create_machine(driver);
		retro_debug_log("mame_execute: after create machine");
		mame = machine->mame_data;
		...
		if (error == 0)
		{
			...
			retro_debug_log("mame_execute: before init machine");
			init_machine(machine);
			retro_debug_log("mame_execute: after init machine");

			/* load the configuration settings and NVRAM */
			retro_debug_log("mame_execute: before config nvram");
			settingsloaded = config_load_settings(machine);
			nvram_load(machine);
			sound_mute(machine, FALSE);
			retro_debug_log("mame_execute: after config nvram");

			/* display the startup screens */
			ui_display_startup_screens(machine, firstrun, !settingsloaded);
			firstrun = FALSE;
			retro_debug_log("mame_execute: after startup screens");
			...
			retro_debug_log("mame_execute: after soft reset");

			/* libretro owns the frame loop and must return to the frontend. */
#ifdef __LIBRETRO__
			return MAMERR_NONE;
#else
