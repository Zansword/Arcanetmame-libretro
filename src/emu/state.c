...
/*-------------------------------------------------
    state_save_get_size - return the exact size of an in-memory state
-------------------------------------------------*/

UINT32 state_save_get_size(running_machine *machine)
{
	state_private *global = machine->state_data;
	state_entry *entry;
...
/*-------------------------------------------------
    state_save_write_buffer - write an uncompressed state
-------------------------------------------------*/

state_save_error state_save_write_buffer(running_machine *machine, void *buffer, size_t size)
{
	state_private *global = machine->state_data;
	UINT32 signature = get_signature(machine);
	...
	if (global == NULL || global->illegal_regs > 0 || size < state_save_get_size(machine))
		return STATERR_WRITE_ERROR;
...
/*-------------------------------------------------
    state_save_read_buffer - restore an in-memory state
-------------------------------------------------*/

state_save_error state_save_read_buffer(running_machine *machine, const void *buffer, size_t size)
{
	state_private *global = machine->state_data;
	UINT32 signature = get_signature(machine);
	...
	if (global == NULL || global->illegal_regs > 0 || size < HEADER_SIZE)
		return STATERR_READ_ERROR;
