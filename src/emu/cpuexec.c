		...
		for (classdata = global->executelist; classdata != NULL; classdata = classdata->next)
		{
			...
			if (target.seconds >= classdata->localtime.seconds)
			{
				...
				if (delta >= classdata->attoseconds_per_cycle)
				{
					...
					{
						#ifdef __LIBRETRO__
						static UINT32 retro_cpu_log_count;
						if (retro_cpu_log_count < 32)
						{
							char retro_cpu_log[96];
							sprintf(retro_cpu_log, "cpuexec: before %s cycles=%d", classdata->device->tag, classdata->cycles_running);
							retro_debug_log(retro_cpu_log);
						}
						#endif
						...
						*classdata->icount = classdata->cycles_running;
						...
						{
							char retro_cpu_log[96];
							sprintf(retro_cpu_log, "cpuexec: after %s ran=%d", classdata->device->tag, ran);
							retro_debug_log(retro_cpu_log);
							retro_cpu_log_count++;
						}
						#endif
